...
```bash
zip project_backup.zip errors.txt logs/config/*.conf logs/**/*.logs archived/old/*.logs
```
...
```bash
{
  echo "app.conf $(wc -l < logs/config/app.conf)"
  echo "db.conf  $(wc -l < logs/config/db.conf)"
} > conf_stats.txt
```
